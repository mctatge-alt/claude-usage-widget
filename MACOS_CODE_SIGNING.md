...
- **Never commit** `.p12`, `developer_id_base64.txt`, or other signing material (see `.gitignore`)
- **Do not hardcode** `APPLE_TEAM_ID` in `package.json` — use `.env` locally and GitHub Actions secrets in CI
- The `.env` file is already in `.gitignore`
- This `MACOS_CODE_SIGNING.md` file is safe to commit (it's just a template)
- If signing material was ever committed to git history, revoke the certificate with Apple and purge history before publishing — see [SECURITY.md](SECURITY.md)
